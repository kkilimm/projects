...
double F_i(double x, int i, int N, double* X){
    double P = 1.0;
...
double L_n(double x, double* X, double* Y, int N, double (*F_i) (double, int, int, double*)){
    double S = 0.0;
...
void print(double* X, double* Y, double* A, int N, std::ofstream& file){
    ...
    for (int i = 0; i < N - 1; i++) {
        ...
        file << X[i] << " " << Y[i] << " " << L_n(X[i], X, Y, N, F_i) << " " << P_nminus1(X[i], A, N) << "\n";

        file << X[i] + h << " " << f(X[i] + h) << " " << L_n(X[i] + h, X, Y, N, F_i) << " " << P_nminus1(X[i] + h, A, N) << "\n";

        file << X[i] + 2*h << " " << f(X[i] + 2*h) << " " << L_n(X[i] + 2*h, X, Y, N, F_i) << " " << P_nminus1(X[i] + 2*h, A, N) << "\n";

    }

    file << X[N - 1] << " " << Y[N - 1] << " " << L_n(X[N - 1], X, Y, N, F_i) << " " << P_nminus1(X[N - 1], A, N) << "\n";
...
int main(){
    ...
    /*for (int i = 0; i < N; i++) {
        printf("%25.15lf ", X[i]);
        printf("%25.15lf ", Y[i]);
        printf("%25.15lf", L_n(X[i], X, Y, N, F_i));
        printf("%25.15lf\n", P_nminus1(X[i], A, N));
    }*/
